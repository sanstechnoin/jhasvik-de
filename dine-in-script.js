...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    firebaseBtn.addEventListener('click', async (e) => {
        ...
        // --- THIS IS THE CHANGE ---
        // We now create a unique ID for EVERY order, so they don't replace each other.
        const orderId = `table-${tableNumber}-${new Date().getTime()}`;
        // --- END OF CHANGE ---

        const orderData = {
            id: orderId, // The new unique ID
            table: tableNumber,
            items: itemsOnly,
            status: "new", // "new" will trigger the popup
            createdAt: firebase.firestore.FieldValue.serverTimestamp() // Get accurate server time
        };

        try {
            // --- THIS IS THE CHANGE ---
            // We use .add() to create a NEW order every time.
            // We are no longer using .set()
